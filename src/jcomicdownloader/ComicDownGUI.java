                                                    ...
                                                    WindowFocusListener
{
    ...
    private void setButton( Container contentPane )
    {
        ...
        //Jimmy, 20150420, 增加自動下載最新話數
        button[ButtonEnum.AUTO] = getButton( "自動", "Auto", "自動下載我的最愛之最新集數", "auto.png", KeyEvent.VK_M );
            ...
            if ( !clipString.equals( Common.prevClipString ) )
            {
                ...
                if ( Common.isLegalURL( clipString ) )
                {
                    ...
                    if ( SetUp.getAutoAddMission() )
                    {
                        ...
                        parseURL( tempArgs, false, false, 0, false );
                    }
                }
            }
            else
                                ...
                                final boolean auto )
    {
    	...
    	if (auto) {
    		ChoiceFrame choiceFrame = new ChoiceFrame( title, urlString, auto );
    	} else {
	        new Thread( new Runnable()
	        {
	                    ...
	                    public void run()
	                    {
	                        ...
	                        if ( modifySelected )
	                        {
	                            choiceFrame = new ChoiceFrame(
	                                    "重新選擇欲下載的集數 [" + title + "]", true, modifyRow, title, urlString, auto );
	                        }
	                        else
	                        {
	                            ...
	                            choiceFrame = new ChoiceFrame( title, urlString, auto );
    ...
    // 分析位址，分析完後開始下載。
    public void singleParseURL( final String[] newArgs,
            ...
            final boolean auto )
    {
        ...
        else
        {
            ...
            else
            {
                ...
                Run mainRun = new Run( tempArgs, RunModeEnum.PARSE_MODE );
                mainRun.singleRun();
                String title = "";
                try
                {
                ...
                { // args is main page
                    runChoiceFrame( modifySelected, modifyRow, title, tempArgs[0], auto );
...
//    	}
    ...
    public void actionPerformed( ActionEvent event )
    {
        ...
        { // button of add
            ...
            parseURL( args, false, false, 0, false );
            args = null;
        ...
        { // button of Download
            ...
            else
            {
                ...
                parseURL( args, true, false, 0, false );
                args = null;
            }
        }
        ...
        if ( event.getSource() == button[ButtonEnum.AUTO]) {
        	Common.debugPrintln("Auto add download list.");

        	
        	for (int i = 0; i < bookmarkTableModel.getRowCount(); i++) {
            	Common.debugPrintln(bookmarkTableModel.getValueAt(i, 1).toString() + " : " + bookmarkTableModel.getValueAt(i, 2).toString());
            	//TODO add list to download
				if ( Flag.downloadingFlag || Flag.parseUrlFlag ) { // 目前正分析網址或下載中，不提供直接下載服務
					...
					args = new String[1];
					args[0] = bookmarkTableModel.getValueAt(i, 2).toString();
					System.out.println("args[0]:"+args[0]);
					singleParseURL( args, false, false, 0, true );
//					parseURL( args, false, false, 0, false );
					args = null;
				}
        	}
