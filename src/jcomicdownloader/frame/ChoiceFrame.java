        ...
        TableModelListener, MouseListener, MouseMotionListener
{
    ...
    private List<String> newestVolume;
    
    public ChoiceFrame( String title, String url, boolean auto )
    {
        this( Common.getStringUsingDefaultLanguage( "選擇欲下載的集數 [", "selected volumes [" ) + title + "]",
              false, 0, title, url, auto );
    }

    public ChoiceFrame( String frameTitle, boolean modifySelected, int modifyRow, String title, String url, boolean auto )
    {
      ...
      //Jimmy, 20150420, 增加自動下載最新話數
        if (auto) {
        	checkStrings = autoSelectedNewestVolume();
        	
        	Common.debugPrintln("title:"+title);
        	for (int i = 0; i < volumeStrings.length; i++) {
            	Common.debugPrintln(volumeStrings[i] + ":" + checkStrings[i] + "(" + urlStrings[i] + ")");
        	}
        	
        	new ActionHandler().confirm();
        	return;
        } else {
        	setVisible( true );
        }
    ...
    //Jimmy, 20150420, No UI check newest volume
    private String[] autoSelectedNewestVolume() {
    	String[] checkStrings = new String[volumeStrings.length];
    	
    	for (int i = 0; i < volumeStrings.length; i++) {
    		String volumeTitle = volumeStrings[i];
